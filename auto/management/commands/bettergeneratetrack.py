...
class Command(BaseCommand):
    ...
    def _create_random_date(self, start_date_str: str, end_date_str: str) -> datetime:
        format = "%Y-%m-%d:%H:%M:%S"
    ...
    def handle(self, *args, **options) -> None:
        ...
        for n in range(count):
            ...
            self.timestamp: datetime = make_aware(
                self._create_random_date("2020-01-01:00:00:00", "2023-01-01:00:00:00")
            )
            ...
            this_ride.save()
            self.stdout.write(self.style.SUCCESS(f"{self.vehicle_id} -- success {n + 1}/{count}"))
